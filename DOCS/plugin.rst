...
* il layer dello strumento urbanistico (PRG o equivalenti) correttamente configurato per l'estrazione delle NTA;
* il percorso a una cartella di output in cui salvare il file .pdf del ;
...
* La colonna contenete il nome dell'area deve essere di **tipo testo** e deve avere **nome o alias** = **nome** o *nome o alias** = **sigla** o *nome o alias** = **zona**(sia il nome che l'alias sono case insensitive ovvero non influenzati dall'uso di lettere maiuscole/minuscole --> è possibile scrivere nome, NOME, Nome, NoMe, ecc.). Questa colonna non è obbligatoria, se il plugin non trova un colonna nominata o con alias = *nome* semplicemente segnalerà con un messaggio di warning che la colonna non è stata trovata nel relativo layer e quindi non stamperà alcun valore nella riga corrispondente del CDU. Qualora invece esistesse una colonna con *nome o alias* = *nome* nella tabella degli attributi, sono ovviamente ammessi eventuali valori nulli; anche in questo caso il plugin non stamperà alcun valore nella riga corrispondente del CDU.
...
Per consentire l'estrazione automatica delle **Norme Tecniche di Attuazione** (NTA) relative alla zona specifica interessata è necessario che il campo articolo dello strumento urbanistico contenga il riferimento all'articolo riportato con la stessa denominazione presente nel documento delle NTA. La corrispondenza tra il valore del campo e il testo delle NTA deve quindi essere univoca e coerente. Nel caso in cui l'area fosse interessata da più articoli, inserire la dicitura artt. 4-6 (per indicare artt. dal 4 al 6) o artt 45 e 46 (per indicare gli articoli 45 e 46). 
...
* **11 - Filtro di Tolleranza Geometrica (Soglia fissa 1 mq):** Questa opzione consente di attivare un filtro topologico automatico nel momento in cui le particelle del catasto si intersecano in minima parte (per piccolissime porzioni o pochi centimetri) con geometrie o zone adiacenti del PRG. Questo fenomeno si riscontra frequentemente a causa di lievi disallineamenti o piccoli errori storici di digitalizzazione tra la mappa catastale e lo strumento urbanistico. All'attivazione della spunta, il plugin esclude dal calcolo del CDU queste micro-intersezioni millimetriche, riducendo gli errori e concentrando l'assegnazione e l'area della zona urbanistica sulla particella dominante (ovvero dove ricade la porzione principale del terreno). Le micro-aree scartate non sporcheranno il certificato ma verranno esposte in modo trasparente all'interno della finestra dei Log per il controllo del tecnico. 
* **12 - Scelta dei Layer per Applicazione Tolleranza:** Al di sotto della spunta principale, compare un riquadro dinamico contenente l'elenco di tutti i layer appartenenti al Gruppo Urbanistico selezionato. L'operatore può inserire una spunta a quadratino solo ed esclusivamente sui singoli layer su cui desidera applicare l'approssimazione della tolleranza (es. sul layer di Zonizzazione del PRG).
* **13 - Seleziona l'algoritmo di intersezione:** da qui è possibile selezionare l'agoritmo con cui verrà eseguita l'intersezione fra le particelle catastali selezionate e i layer dello strumento urbanistico. Di default viene selezionato l'algoritmo *Clip* di QGIS, più veloce ma più sensibile a eventuali geometrie non valide. In alternativa è possibile selezionare l'algoritmo *Clip by polygon* di GDAL che in molti casi riesce a processare anche le geometrie con problemi di validità aumentando però i tempi di calcolo. Come già detto, in generale si consiglia comunque di utilizzare layer con geometrie valide e topologicamente corrette. In caso di geometrie con errori infatti, è possibile che anche l'algoritmo *Clip by polygon* di GDAL non riesca a portare a termine il processo. Qualora i problemi delle geometrie non permetessero di portare a termine il processo di intersezione, verrà visualizzato nell'area di Log un messaggio di errore e il CDU non sarà creato.
* **14 - Generazione delle NTA (Integrazione Allegato):** Attivando questa opzione e selezionando il percorso del documento pdf delle NTA, il plugin rileva in automatico la zona e l'articolo individuato e preso in esame sulla particella. Estrae in modo mirato dal PDF le pagine contenenti la disciplina specifica di quell'articolo e le unisce direttamente in coda al certificato CDU finale. Le pagine saranno precedute da un frontespizio grafico coordinato nel font che avvisa il lettore riguardo all'articolo estratto e dichiara eventuali altri articoli presenti nella stessa pagina come puramente di contorno. All'attivazione del comando, una finestra pop-up ricorderà all'operatore i requisiti minimi di struttura del database e dei PDF (che devono essere ricercabili).
.. image:: img/nota info.png
...
* **27 - Messaggi di Log:** tutti i messaggi di warning o di errore così come i messaggi informativi sull'andamento del processo compariranno in questa area.
* **28 - Pulisci Log:** il bottone pulisce l’area dei messaggi di Log rimuovendo eventuali messaggi di processi precedenti.
* **29 - Guida:** il bottone apre questo manuale in un web browser.
* **30 - OK:** il bottone lancia il processo. Come già detto, se il plugin restituisce un messaggio di errore il processo viene bloccato e l’utente potrà inserire o modificare i parametri in input, premendo nuovamente il bottone OK il processo verrà rilanciato.
* **30 - Chiudi:** il bottone chiude l’interfaccia grafica del plugin e i parametri in input numero foglio, numero particella e gruppo sono riinizializzati.
...
* le quattro colonne nei layers dello strumento urbanistico **NON sono obbligatorie** per la redazione del certificato standard; tuttavia, la compilazione accurata della colonna articolo diventano requisiti indispensabili nel caso in cui l’utente scelga di attivare la funzione di generazione automatica degli allegati NTA.
...
Prima di avviare il processo, l'utente può configurare le nuove opzioni avanzate introdotte nell'interfaccia:
* **Filtro Tolleranza di 1 mq**: Spuntando la relativa casella, si attiverà un menu a tendina dinamico che mostra l'elenco dei layer presenti nel gruppo urbanistico selezionato, consentendo di scegliere su quale layer specifico applicare la tolleranza ed escludere le micro-geometrie dal computo frammentato.
* **Inclusione Estratto NTA**: È possibile attivare l'opzione spuntabile per includere le NTA della zona di riferimento. All'attivazione del check, comparirà una finestra informativa che ricorda le modalità operative (inserimento del numero dell'articolo nella colonna dedicata del layer o tramite tabella di supporto config_nta in Join).
Una volta definiti i vari parametri in input, premendo il tasto OK il processo sarà avviato. Eventuali messaggi di warning o informativi verranno stampati nell’area di Log. In caso di problematiche che compromettono la corretta compilazione del CDU, il processo si interrompe restituendo un messaggio che descrive l'errore riscontrato; al contrario, se l'anomalia riguarda solo il modulo NTA (es. percorso errato o PDF mancante), il plugin applicherà una logica di fail-safe portando comunque a termine con successo la generazione del CDU standard.
